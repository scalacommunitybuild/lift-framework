...
class JIntCodec extends Codec[JInt] {
  override def decode(reader: BsonReader, decoderContext: DecoderContext): JInt = {
    JInt(reader.readInt64())
  }

  override def encode(writer: BsonWriter, value: JInt, encoderContext: EncoderContext): Unit = {
    writer.writeInt64(value.values.toLong)
  }

  override def getEncoderClass(): Class[JInt] = {
    classOf[JInt]
  }
}
